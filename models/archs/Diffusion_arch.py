...
class NoiseDiffNet(nn.Module):
    def __init__(
        ...
        # MLP for shot noise
        self.shot_mlp1 = Mlp(in_features=4*2, hidden_features=dim, out_features=dim, act_layer=nn.GELU)
        self.shot_attn = AttnBlock(query_dim=dim, context_dim=iso_dim, heads=4, dim_head=32, dropout=0.)
        self.shot_mlp2 = Mlp(in_features=dim, hidden_features=dim, out_features=dim, act_layer=nn.GELU)
        self.shot_time = ResnetBlock(dim, dim, time_emb_dim=time_dim, groups=2, ks=1, pd=0)
        self.shot_mlp3 = Mlp(in_features=dim, hidden_features=dim, out_features=4, act_layer=nn.GELU)
    ...
    def forward(self, x, time, condition = None):
        ...
        shot_emb = self.shot_mlp1(torch.cat([clean_img, x], dim=1))
        r = shot_emb.clone()
        shot_emb = self.shot_attn(shot_emb, iso_emb)
        shot_emb = self.shot_mlp2(shot_emb)
        shot_emb = self.shot_time(shot_emb, t)
        shot_emb = shot_emb + r
        shot_noise = self.shot_mlp3(shot_emb)
        ...
        read_noise = self.final_conv(x)
        out = shot_noise + read_noise
