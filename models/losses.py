...
class VGGStyleDiscriminator160(nn.Module):
    ...
    def __init__(self, num_in_ch=4*2, num_feat=64):
...
class AdversarialLoss(nn.Module):
    def __init__(self, use_cpu=False, gpu_ids=[], dist=False, gan_type='RGAN', gan_k=2,
        ...
        self.discriminator = DiscriminatorS2d_sig(channels = 8).to(self.device)
        if dist:
        ...
        self.optimizer = torch.optim.Adam(
                ...
                betas=(0.5, 0.999), eps=1e-8, lr=lr_dis
            )
    ...
    def forward(self, fake, real):
        ...
        for _ in range(self.gan_k):
            ...
            # gradient penalty
            d_gp_loss = gradient_penalty_loss(self.discriminator, real, fake, weight=None)
            d_gp_loss.backward()
            
            loss_d = d_real_loss + d_fake_loss + d_gp_loss
